...
{
    Common::stUserSigninInfo stInfo;

    stInfo.strName            = ui->nameLineEdit->text();
    stInfo.strGender          = ui->sexComboBox->currentText();
    stInfo.strAge             = ui->ageLineEdit->text();
    stInfo.strProfession      = ui->professionEdit->text();
    stInfo.strPhone           = ui->contactEdit->text();
    stInfo.strPlace           = ui->addressEdit->text();
    stInfo.strPersonalHistory = ui->personalHistoryEdit->toPlainText();
    stInfo.strFamilyHistory   = ui->familyHistoryEdit->toPlainText();
    stInfo.strComment         = ui->addtextEdit->toPlainText();

    emit SendUserInfoContent(stInfo);

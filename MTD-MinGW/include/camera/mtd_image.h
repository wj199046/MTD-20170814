...
public:
    ...
    QImage getPalette();//获取调色盘

    QImage getPartialEnhance(int p1x,int p1y,int p2x,int p2y);//获取局部增强区域,参数为转换后的坐标,不可调参数

    MTDImage getMTDPartialEnhance(int p1x,int p1y,int p2x,int p2y);//获取局部增强区域,参数为转换后的坐标,是原图局域的拷贝

    double* getTemperature(int p1x,int p1y,int p2x,int p2y);//获取局部增强区域,参数为转换后的坐标
private:
